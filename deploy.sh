...
aws --region eu-west-1 lambda update-function-code --function-name "$functionName" --zip-file "fileb://target/scala-2.11/slack-sodexo-assembly-0.1.1-SNAPSHOT.jar" || {
    ...
    aws --region eu-west-1 lambda create-function --timeout 180 --function-name "$functionName" --runtime "java8" --role "arn:aws:iam::285072396330:role/lambda_basic_execution" --handler "de.is24.Sodexo::handleCall" --zip-file "fileb://target/scala-2.11/slack-sodexo-assembly-0.1.1-SNAPSHOT.jar"
}
aws --region eu-west-1 lambda update-function-configuration --function-name "$functionName" --memory-size 256 --timeout 180
